...
def battle_engine():
    # References to properties in hero dict
    # (should the_hero be his own file. Yes, will be a class eventually)
    h_atk_pow = the_hero['attack_power']
    h_hp = the_hero['hp']
    h_wpn = the_hero['weapon']
    h_xp = the_hero['xp']
    h_gold = the_hero['gold']
    health_potion_count = the_hero['inventory'].count('health_potion')

    # Quest List for the current zone
    quest_list = quests["Fairmeadow"]

    random_index = random.randint(0,len(quests["Fairmeadow"])-1)

    # we need to use the copy() method so we don't change the original
    chosen_quest = quest_list[random_index].copy()
    quest_name = chosen_quest['name']
    quest_list.remove(chosen_quest)
    
    in_fight = True #boolean that keeps us in the fight loop
    print(Fore.BLUE+ "Could be...")
    sleep(1.5)
    print(Fore.LIGHTMAGENTA_EX+f"{chosen_quest['backstory']}")
    quest_accepted = input(Fore.MAGENTA+f"Accept Quest: {quest_name}? (y/n) > ")

    if quest_accepted.lower() == 'y':
        pass
    # while in_fight:
        
    #     enemy_attack_modifier = 1
    #     print(f"You have {h_hp} health points remaining \n The {enemy_encountered['name']} has {enemy_encountered['hp']}")
    #     print(f"1. Attack using {h_wpn}")
    ...
    #         damage_dealt = h_atk_pow - enemy_encountered['defense']
    ...
    #             h_xp += enemy_encountered['xp_drop']
    #             h_gold += enemy_encountered['gold_drop']
